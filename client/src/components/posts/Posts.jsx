                            ...
                            {posts.length > 0 ? (
                                posts.map((post) => (
                                    <div key={post._id} className="relative [&:not(:last-child)]:mb-6 p-5 rounded-lg shadow-md border border-base-300 bg-base-100">
                                                    ...
                                                    <div className="space-y-2">
                                                        {post.media.images.filter(url => url && url.trim() !== "").map((url, index) => (
                                                            <img
                                                                ...
                                                                className="rounded-xl w-full max-h-[500px] object-cover border border-base-300 shadow-sm"
                                                    ...
                                                    <div className="space-y-2">
                                                        {post.media.videos.filter(url => url && url.trim() !== "").map((url, index) => (
                                                            <video
                                                                key={`video-${index}`}
                                                                controls
                                                                className="rounded-xl w-full max-h-[500px] bg-black"
                                                            >
                                                                <source src={url} />
                                        ...
                                        {/* Post Meta Info (Simplified) */}
                                        <div className="text-xs text-base-content/40 flex gap-4 mt-3">
                                            <p>{new Date(post.createdAt).toLocaleString()}</p>
                                            {post.featured && <p>🔥 Featured</p>}
                                        </div>
                                    </div>
                                ))
                            )
